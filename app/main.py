...
def require_api_key(authorization: str | None):
    """Require Bearer API key when API_KEY is configured."""
    if not API_KEY:
        return
...
def _build_inputs(req: ChatCompletionRequest):
    ...
    return input_ids.to(DEVICE), input_ids.shape[1]
...
def chat_completions(
    ...
    # Enforce the hard maximum from MAX_TOKENS.
    # A caller asking for more than the configured ceiling gets silently
    # reduced to the maximum allowed value.
